...
function addNewClientFromCmdLineArgs(){
    # $1 should be client ip
    # $2 should be client name
    
    ENDPOINT="vpn.jdt.engineering:60436"
    ...
    echo "Adding new client with IP: $CLIENT_WG_IPV4"
    echo "Adding new client with name: $CLIENT_NAME"
...
function manageMenu() {
    ...
    echo "   5) Exit"
    echo "   6) Add new client from command line args"
    ...
    case "${MENU_OPTION}" in
        ...
        ;;
        6)
            addNewClientFromCmdLineArgs $1 $2
        ;;
    esac
}
